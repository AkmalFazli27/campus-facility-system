...
3. `main` = siap kumpul/demo. `develop` = integrasi harian (harus selalu bisa `npm run dev`).
...
chore: add multer upload limits (5MB, jpeg/png/webp)
```
...
Reviewer cek: sesuai US, validasi ganda, RBAC, error handling, tidak ada password, test/bukti ada. Komentar harus ditindaklanjuti pemilik PR.
...
Artefak Drive: zip `source/` + `database/schema.sql` + `database/seed.sql` + `README.md` + `.env.example` (tanpa `.env`, tanpa `node_modules`, tanpa `public/uploads/*`).
